...
p.innerHTML="This is the example for the pagination"
body.appendChild(p);
...
thead.setAttribute("class", "head1");
thead.setAttribute("id", "head1");
document.getElementById("table").appendChild(thead);

let tr = document.createElement("tr");
tr.setAttribute("id", "myTr");
document.getElementById("head1").appendChild(tr);

let th1 = document.createElement("th");
let x = document.createTextNode("ID");
th1.appendChild(x);
document.getElementById("myTr").appendChild(th1);

let th2 = document.createElement("th");
let y = document.createTextNode("NAME");
th2.appendChild(y);
document.getElementById("myTr").appendChild(th2);

let th3 = document.createElement("th");
let z = document.createTextNode("EMAIL");
th3.appendChild(z);
document.getElementById("myTr").appendChild(th3);
...
div3.setAttribute("class","d-flex justify-content-center");
...
function pageButtons(pages) {
    ...
    if (state.page != 1) {
        wrapper.innerHTML = `<button value=${maxLeft} class="page btn btn-sm btn-info">&#171; Previous</button>` + wrapper.innerHTML
    }
    ...
    if (state.page != pages) {
        wrapper.innerHTML += `<button value=${maxRight} class="page btn btn-sm btn-info">Next &#187;</button>`
    }
    ...
    var pagec = document.querySelectorAll('.page');

    for (let i = 0; i <= pagec.length; i++) {
        if (pagec[i]) {
            pagec[i].addEventListener('click', function () {
                document.getElementById('table-body').replaceChildren();
                state.page = Number(pagec[i].value)
                buildTable()
            });
        }
    }
